...
from tprpy import keys
from tprpy.contract import Contract, ShieldedTRC20, ContractMethod
from tprpy.keys import PrivateKey
from tprpy.providers import HTTPProvider
from tprpy.defaults import conf_for_name
from tprpy.exceptions import (
    BadSignature,
    BadKey,
    BadHash,
    BlockNotFound,
...
class TransactionRet(dict):
    def __init__(self, iterable, client: "Tpr", method: ContractMethod = None):
...
class Transaction(object):
    ...
    def __init__(self, raw_data: dict, client: "Tpr" = None, method: ContractMethod = None):
    ...
    def broadcast(self) -> TransactionRet:
        """Broadcast the transaction to TPR network."""
...
class TransactionBuilder(object):
    ...
    def __init__(self, inner: dict, client: "Tpr", method: ContractMethod = None):
        self._client = client
        self._raw_data = {
            "contract": [inner],
...
class Trx(object):
    ...
    def account_permission_update(self, owner: TAddress, perm: dict) -> "TransactionBuilder":
        ...
        :param perm: Permission dict from :meth:`~tprpy.Tpr.get_account_permission`
        """
        return self._build_transaction(
...
class Tpr(object):
    ...
    :param provider: An :class:`~tprpy.providers.HTTPProvider` object, can be configured to use private node
    :param network: Which network to connect, one of ``"mainnet"``, ``"shasta"``, ``"nile"``, or ``"tronex"``
    """

    # Address API
    is_address = staticmethod(keys.is_address)
    """Is object a TPR address, both hex format and base58check format."""
    ...
    def list_accounts(self, offset: int, limit: int = 100) -> list:
        """List all accounts"""
        # NOTE: visible parameter is ignored
        ret = self.provider.make_request("wallet/listaccounts", {"visible": True, "offset": offset, "limit": limit})
        accounts = ret.get("accounts", [])
        for account in accounts:
            account["address"] = keys.to_base58check_address(account["address"])
