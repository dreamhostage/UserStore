...
from tprpy.keys import to_base58check_address, is_address, to_tvm_address


class TprAddressDecoder(Fixed32ByteSizeDecoder):
    value_bit_size = 20 * 8
    is_big_endian = True
...
class TprAddressEncoder(Fixed32ByteSizeEncoder):
    value_bit_size = 20 * 8
...
def do_patching(registry):
    ...
    registry.register(
        BaseEquals("address"), TprAddressEncoder, TprAddressDecoder, label="address",
    )
